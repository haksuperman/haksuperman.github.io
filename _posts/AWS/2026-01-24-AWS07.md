...
예를 들어, S3 Bucket에 파일을 쓰는 S3 메서드인 PutObject를 호출할 수 있는 권한을 해당 객체에 부여하지만, 다른 S3 Bucket에서 PutObject를 호출하는 것은 허용하지 않는 것이다. (my-bucket-name 객체에만 s3:* 권한을 부여한 것)
...
- `SourceIp`가 17.5.6.7인 경우, 모든 리소스에 `s3:PutObject` 작업 허용
- `CurrentTime`이 2020-01-01 자정 이후 전체 리소스에 `S3:PubObject` 작업 허용
...
```
위 정책의 경우, `ec2:TerminateInstances`를 제외한 모든 EC2 작업을 부여한다고 생각할 수 있다.<br>
...
반면 예외의 리소스가 있다. 바로 S3 bucket에 대한 리소스 정책에는 여전히 필요하다. S3 bucket 자체를 지정하는 것인지, 그 안의 객체 파일을 지정하는 것인지에 대한 구분이 필요하다.
...
### 3.5. 적용 가능 리소스
일반적으로 S3 bucket, KMS key, IAM role, Lambda 함수와 같은 리소스에 적용 가능하다. (모든 리소스에 적용 가능한 것은 아님)
...
2. 그룹 사용
	...
	- 총 14번의 API 호출 필요 (그룹 생성 1 + 멤버 추가 10 + 정책 연결 3)<br>
...
1. 신뢰 정책(Trust Policy) 설정 - Alice를 신뢰할 수 있는 보안 주체로 새 역할을 생성한다. (Alice가 AssumeRole을 호출할 수 있도록)
...
- resourcepolicy.json
- Effect : 허용 / Action : sts:AssumeRole / Principal : Alice 유저
	```json
	{
	  "Version": "2012-10-17",
	  "Statement": [
...
> IAM User(Alice)가 IAM Role을 요청할 수 있는 권한이 부여된 것이다.<br>
> 이제 신뢰 정책(AssumeRole 호출이 가능한 IAM Role)이 있으므로, 관리형 정책을 생성하고 연결해 IAM Role에 권한을 부여할 수 있다.
{: .prompt-info }
